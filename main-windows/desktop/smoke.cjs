...
async function run(win){
 ...
 report.localization=await require("./i18n-smoke.cjs").run(win);report.tests.push(...report.localization.tests);report.consoleErrors=errors;report.ok=report.tests.every(t=>t.ok)&&!errors.length;return report;
}
module.exports={run};
