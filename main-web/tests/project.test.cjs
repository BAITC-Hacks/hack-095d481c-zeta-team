'use strict';
const assert=require('node:assert/strict'),PF=require('../assets/planner.js'),Project=require('../assets/project.js');
...
test('Manual quantities survive; approvals require confirmation after open',()=>{const s={...state,edits:{demo0:60},approved:new Set(['Поставщик А'])},target={};Project.restore(target,Project.decode(Project.encode(s)),PF);assert.equal(target.edits.demo0,60);assert.equal(target.approved.size,0);});
...
test('Original sheets are omitted; import diagnostics survive',()=>{const s={...state,sheets:[{name:'Тест',file:'тест.xlsx',rows:[['SKU','Qty'],['A001',123]]}],errors:[{name:'test',error:'invalid'}]};const target={};Project.restore(target,Project.decode(Project.encode(s)),PF);assert.deepEqual(target.sheets,[]);assert.deepEqual(target.errors,s.errors);});
