...
const date=s=>typeof s==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(s)&&Number.isFinite(Date.parse(s+'T00:00:00Z'));
...
function validate(p){
 ...
 for(const [key,x] of Object.entries(p.data.products)){
  ...
  for(const k of ['name','sku','code','category'])ensure(typeof x[k]==='string','нет поля '+k+' у товара.');
...
function encode(S){
 const p={format:FORMAT,schema:1,appVersion:'0.2.0',savedAt:new Date().toISOString(),lastCalc:new Date(S.lastCalc).toISOString(),data:S.data,cfg:S.cfg,sheets:S.sheets,errors:S.errors,edits:S.edits,approved:[...S.approved]};
...
function restore(S,p,PF){
 ...
 S.approved=new Set(p.appVersion==='0.2.0'?p.approved:[]);S.lastCalc=new Date(p.lastCalc||p.savedAt);S.page=0;S.filter='all';S.search='';S.category='';S.supplier='';
 return S;
}
